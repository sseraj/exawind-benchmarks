...
data = np.loadtxt('expq3.dat')
cloc = data[:, 0]
cval = data[:, 1]
nval = len(cloc)

data = np.loadtxt('expa3.dat')
cloc2 = data[:, 0]
cval2 = data[:, 1]*1000
nval2 = len(cloc2)

data = np.loadtxt('workq3.dat')
cloc3 = data[:, 0]
cval3 = data[:, 1]*1000
nval3 = len(cloc3)
cval4 = data[:, 2]*1000

data = np.loadtxt('nalu3.dat')
...
ax.text(1.0, 20, r"AoA = $3^\circ$")

##################  OUTPUT  ##################

plt.savefig('aoa3.png', format='png', dpi=200,bbox_inches=0)
